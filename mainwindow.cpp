...
void MainWindow::on_actionDelete_triggered()
{
    ...
    if(choice == QMessageBox::Yes){
        ...
        close_database();
        ui->labelTotal->setText("Total: " + format.toCurrencyString(0.0));
...
{
    // Create a new instance of your dialog
    Dialog *dialog = new Dialog(this);

    // Show the dialog and wait for the user to interact with it
    if (dialog->exec() == QDialog::Accepted) {
        // The user clicked the "Ok" or "Yes" button
            QDateEdit *to = dialog->findChild<QDateEdit*>("dateEdit_2");
            QDateEdit *from = dialog->findChild<QDateEdit*>("dateEdit_1");
            QComboBox *comboBox = dialog->findChild<QComboBox*>("comboBox");
        ...
        if (selectedItem == "Both") {
            query = QString("SELECT description, mode, trans_amount, balance, date_added FROM transactions WHERE date_added BETWEEN '%1' AND '%2' AND (mode = 'Deposit' OR mode = 'Withdraw');")
                .arg(fromDate.toString("yyyy-MM-dd"))
                .arg(toDate.toString("yyyy-MM-dd"));
        } else {
            query = QString("SELECT description, mode, trans_amount, balance, date_added FROM transactions WHERE date_added BETWEEN '%1' AND '%2' AND mode = '%3';")
                .arg(fromDate.toString("yyyy-MM-dd"))
                .arg(toDate.toString("yyyy-MM-dd"))
                .arg(selectedItem);
        }
        qDebug() << query;
        view_all_transactions_model->setQuery(query, transaction_db);
